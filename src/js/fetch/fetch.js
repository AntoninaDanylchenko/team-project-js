...
const on = {
  stringSearchButtonClick: ref.querySearch.addEventListener('submit', onSearchButtonClick),
...
function getVideoByID(movieID = 414419) {
  ...
  const request = findMovies.find()
    .then(function(answer) {
      ...
      console.log(answer);// у відповідь отримуємо об'єкт, який для прикладу консолимо.
      console.log('site:', answer.results[0].site, 'key:', answer.results[0].key, `youtube link: https://youtu.be/${answer.results[0].key}`);
    });
}
